...
void MidData(struct Node* prev_node, int data) {
	// check to see of the 'prev_node' is valid (!NULL)
	if (prev_node == NULL) {
		//called a mid push when it should have been a append to the NULL list found
		AppendData(&prev_node, data);
...
/*  Recursively search a linked list for a certain data value   */
void SearchListFor(struct Node* head, int data) {
	// if list is just NULL then nothing to look for, list is empty
	//  of
	// searched until we are pointing at NULL/end-of-list
	if (head == NULL) {
		printf("Item %d not found.", data);
		return;
	}

	// data found, inform user and return
	if (head->data == data) {
		printf("Item %d found at list address %p", data, &head);
		return;
	}

	// keep looking by pointing to the next list member
	SearchListFor(head->next, data);
}
